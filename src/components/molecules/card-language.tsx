...
export const CardLanguage: React.FC<CardLanguageProps> = ({ languages }) => {
  ...
  const filteredLanguages = languages.filter(
    (language) =>
      language.title || language.live || language.textTopic || language.textLink
  );
      ...
      {filteredLanguages.map((language) => (
        ...
        >
          ...
          <div className="flex flex-col justify-between gap-2 h-full">
            {(language.textTopic || language.topic || language.date) && (
              <div className="text-slate-500 space-y-1 mb-4">
                {language.textTopic && <span>{language.textTopic}</span>}{" "}
                {language.topic && (
                  <span className="font-bold text-sm">"{language.topic}"</span>
                )}
                {language.date && (
                  <div className="text-xs">{language.date}</div>
                )}
              </div>
            )}
            {!language.date && (
          ...
          <div className="flex justify-center mt-auto">
            <Button variant="blue">{language.textLink || " "}</Button>
          </div>
        </div>
      ))}
    </>
